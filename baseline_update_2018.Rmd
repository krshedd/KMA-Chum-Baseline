...
populations_63_info <- tibble(pop = collections_post_pooling) %>% 
  ...
  mutate(pop_no_UW = recode(pop_no_UW, !!!list("CMBARLR15E" = "CMBARLR15", "CMBARLR15M" = "CMBARLR15", "CMBARLR15L" = "CMBARLR15"))) %>%  # remove E, M, L extension for Barling 2015 for joining
  ...
  select(pop, Location, `Collection Date 1`, `Collection Date 2`, `Collection Date 3`, Month, Quadrant, n, Latitude, Longitude, color) %>%  # keep columns
  ...
  select(-trash, -more_trash) %>%  # drop bogus location info
...
icons <- awesomeIcons(icon = 'ios-close', iconColor = 'black', library = 'ion', markerColor = populations_63_info$color)
...
populations_63_info <- populations_63_info %>% 
  ...
  rename(Group = Quadrant) %>% 
  select(map_no, pop, Location, `Collection Date 1`, `Collection Date 2`, `Collection Date 3`, Month, Group, n, Latitude, Longitude, color) %>% 
  arrange(map_no)

write_csv(populations_63_info, "../2018/Tables/populations_63_info.csv")

populations_63 <- populations_63$pop
save_objects(c("populations_63", "populations_63_info"), "../2018/Objects/")
```
...
populations_63_info %>% 
  leaflet(width = "100%") %>% 
  addTiles() %>% 
  addAwesomeMarkers(~ Longitude, ~ Latitude, icon = icons, label = ~ as.character(map_no), labelOptions = labelOptions(noHide = TRUE, direction = "top"))
```
...
# Need appropriate groupvec
# allele_freq <- FreqFisPlot4SNPs.GCL(sillyvec = populations_63, loci = loci91[-c(90:91)], )
```
...
pairwise_fst <- get_lower_tri(Fst_tree_populations_63_loci91$PairwiseFst) %>% 
